...
class DeepLinkNavArgsProcessor : AbstractProcessor() {
    ...
    private companion object {
        ...
        private const val newClassNameSuffix = "BindingArgs"
    }
    ...
    private fun createBindingFile(packageElement: PackageElement, dataClassElement: Element, fields: List<Element>): Boolean {
        ...
        val newFileSpec = FileSpec.builder(newClassName)
            ...
            .addImport("kotlin.reflect.full", "memberProperties",
                "primaryConstructor", "starProjectedType").addType(newClass.addAnnotation(annotation).addType(companionObject)
                .addFunction
                    (methods).build())
            .build()
    ...
    private fun createPrimaryConstructor(fields: List<Element>, newClassName: ClassName): TypeSpec.Builder {
        ...
        fields.forEach { field ->
            constructorBuilder.addParameter(
                ParameterSpec.builder(field.simpleName.toString(), if (field.asType().asTypeName().toString() == String::class.java.name)
                    String::class.asTypeName() else field.asType().asTypeName()).build())
        }

        val properties = fields.map { field ->
            PropertySpec.builder(field.simpleName.toString(), if (field.asType().asTypeName().toString() == String::class.java.name)
                String::class.asTypeName() else field.asType().asTypeName()).initializer(field.simpleName.toString()).build()
        }
...
import kotlin.reflect.full.starProjectedType
    ...
    /**
     * return map of properties on this class
