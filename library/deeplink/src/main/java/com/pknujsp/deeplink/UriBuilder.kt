...
internal fun toQueryUri(deepLinkUrl: String, parameter: Map<String, Any>): Pair<Uri, Uri> {
    val baseUri = WeakReference(StringBuilder(deepLinkUrl)).get()!!.let { uri ->
        parameter.keys.forEachIndexed { index, key ->
            uri.append("$key={$key}")
    ...
    val queryUri = WeakReference(baseUri.toString()).get()!!.let { uri ->
        parameter.forEach { (key, value) ->
            uri.replace("{$key}", value.toString())
        }
        uri.toUri()
    }
